...
public class PostcodeService implements IPostcodeService {
    ...
    public DistanceDTO getDistance(DistanceRequestDTO requestDTO) {
        ...
        Double distanceResult = CalculationUtil.calculateDistance(postcodeFrom.latitude(), postcodeFrom.longitude(), postcodeTo.latitude(), postcodeTo.longitude());
        log.info("Distanace Result: {}", distanceResult);
        
        DistanceDTO distanceDTO = new DistanceDTO(requestDTO.postcodeFrom(), requestDTO.postcodeTo(), distanceResult, "KM");
        return distanceDTO;
    }
}